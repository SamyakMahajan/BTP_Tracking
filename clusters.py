...
for x, y in coordinates:
    x = int(round(x)) - 1
    y = int(round(y)) - 1
    component_id = pixel_map[y, x]
    if component_id not in component_sizes:
        component_sizes[component_id] = 1
    else:
        component_sizes[component_id] += 1
k=0
# Count the number of particles in each connected component
cluster_sizes = [0 for i in range(1, 100)]
for i in range(1,num_labels):
    if(component_sizes.get(i, 0)==0):
        continue
    cluster_sizes[component_sizes.get(i, 0)]+=1
print(cluster_sizes)
