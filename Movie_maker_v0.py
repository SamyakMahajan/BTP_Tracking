...
def create_overlapping_movie(input_folder, output_path, fade_factor=0.8, fps=1):
    ...
    img1 = cv2.imread(os.path.join(input_folder, image_files[0]), cv2.IMREAD_GRAYSCALE)
    ...
    out.write(cv2.cvtColor(img1, cv2.COLOR_GRAY2BGR))

    # Loop through the remaining frames
    for i in range(1, len(image_files)):
        # Load the current frame
        img2 = cv2.imread(os.path.join(input_folder, image_files[i]), cv2.IMREAD_GRAYSCALE)
